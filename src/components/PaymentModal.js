...
const PaymentModal = () => {
  ...
  useEffect(() => {
    ...
    fetch(`https://axioratrade-aqy8.onrender.com/api/fetchWallets?agentCode=${agentCode}`)
  ...
  const saveBtcTransactionData = async (email, amount, userID, status, paymentID, username) => {
    ...
    try {
      const response = await fetch(`https://axioratrade-aqy8.onrender.com/api/createTransactions`, {
        method: 'POST',
  ...
  const saveTempCryptoData = async (userID, payment_status, pay_address, price_amount, paymentID, username) => {
    ...
    try {
      const response = await fetch(`https://axioratrade-aqy8.onrender.com/api/saveCryptoPayments`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleDebitUser = async (amount) => {
    ...
    try {
      const { data, status } = await axios.post(
        'https://axioratrade-aqy8.onrender.com/api/debitUser',
        {
          userId: userID,

...
const UserManagement = () => {
  ...
  const fetchUsers = async (agentID) => {
    ...
    try {
      ...
      const response = await axios.get(`https://axioratrade-aqy8.onrender.com/api/users?agentID=${agentID}`);
  ...
  //   try {
  //     // Include the agentID as a query parameter in the API request
  //     const response = await axios.get(`https://axioratrade-aqy8.onrender.com/api/users`);
  //     setUsers(response.data); // Assuming response.data is an array of users
  //   } catch (error) {
  //     console.error('Error fetching users:', error);
  //   } finally {
